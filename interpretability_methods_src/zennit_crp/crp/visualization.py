...
from interpretability_methods_src.zennit_crp.crp.maximization import Maximization
from interpretability_methods_src.zennit_crp.crp.concepts import ChannelConcept, Concept
from interpretability_methods_src.zennit_crp.crp.statistics import Statistics
from interpretability_methods_src.zennit_crp.crp.hooks import FeatVisHook
from interpretability_methods_src.zennit_crp.crp.helper import load_maximization, load_statistics, load_stat_targets
from interpretability_methods_src.zennit_crp.crp.image import vis_img_heatmap, vis_opaque_img
from interpretability_methods_src.zennit_crp.crp.cache import Cache
...
class FeatureVisualization:
    ...
    def cache_reference(func):
        """
        Decorator for get_max_reference and get_stats_reference. If a interpretability_methods_src.zennit_crp.crp.cache object is supplied to the FeatureVisualization object,
    ...
    def get_max_reference(
        ...
        If the interpretability_methods_src.zennit_crp.crp.concept class (supplied to the FeatureVisualization layer_map) implements masking for a single neuron in the 'mask_rf' method, 
        ...
        rf: boolean
            If True, compute the interpretability_methods_src.zennit_crp.crp heatmap for the most relevant/most activating neuron only to restrict the conditonal heatmap
    ...
    def get_stats_reference(self, concept_id: int, layer_name: str, targets: Union[int, list], mode="relevance", r_range: Tuple[int, int] = (0, 8),
        ...
        In addition, conditional heatmaps can be computed on reference samples. If the interpretability_methods_src.zennit_crp.crp.concept class (supplied to the FeatureVisualization layer_map) 
    ...
    def precompute_ref(self, layer_c_ind:Dict[str, List], composite: Composite, rf=True, stats=False, top_N=4, mean_N=10, mode="relevance", r_range: Tuple[int, int] = (0, 8), plot_list=[vis_opaque_img], batch_size=32):
        ...
        if self.Cache is None:
            raise ValueError("You must supply a interpretability_methods_src.zennit_crp.crp.Cache object to the 'FeatureVisualization' class to precompute reference images!")

...
class ModelGraph:
    ...
    def __init__(self, input_nodes):
        ...
        self.input_nodes = set(self.layer_node_map.values())
    ...
    def exclude_parallel_layers(self, conditions: List[Dict[str, List]], MODEL_OUTPUT_NAME="y"):

        conditions = conditions.copy()
        for cond in conditions:

            layer_cond = list(cond.keys())
            if MODEL_OUTPUT_NAME in layer_cond:
                layer_cond.remove(MODEL_OUTPUT_NAME)

            shortest_path = self._find_shortest_path(layer_cond)

            exclude = set(self.layer_names) ^ set(shortest_path)

            for l_name in exclude:
                cond[l_name] = []

        return conditions

    def _find_shortest_path(self, layer_names):

        shortes_path = set()
        dest_names = layer_names + [node.layer_name for node in self.input_nodes]

        for l_name in dest_names:
            print(l_name, len(dest_names))
            dest_connect = set()
            out_connect = set()

            root_node = self.layer_node_map[l_name]
            self._find_connection(root_node, dest_names, dest_connect, out_connect)

            if len(dest_connect) > 0:
                shortes_path.update(dest_connect)
            elif len(out_connect) > 0:
                shortes_path.update(out_connect)
            else:
                raise RuntimeError("graph search could not converge.")

        shortes_names = set(node.layer_name for node in shortes_path if node.layer_name in self.layer_names)

        return shortes_names

    def _find_connection(self, root_node, dest_l_names: set, dest_connections: set, out_connection: set):

        connected = False
        for g_node in root_node.output_nodes:

            # print(g_node)
            # print(g_node.layer_name)
            if g_node.layer_name in dest_l_names:
                # direct connection
                dest_connections.add(root_node)
                return True

            elif len(g_node.output_nodes) == 0:
                # return no direct connection
                out_connection.add(root_node)
                return False

            result = self._find_connection(g_node, dest_l_names, dest_connections, out_connection)
            if result is True:
                dest_connections.add(root_node)
                connected = True
            else:
...
def trace_model_graph(
        model, input: Union[Tuple[int, ...],
                            torch.Tensor],
        layer_names: List[str],
        verbose=False) -> ModelGraph:
    """"
    ...
    ----------
        ...
        input: tuple of int or torch.Tensor
            An examplary input. Used to trace the model with torch.jit.trace.
            If a tuple is supplied, a torch.ones tensor is created, otherwise the supplied
            tensor will be used as input
    ...
    if isinstance(input, tuple):
        sample = torch.ones(input).to(next(model.parameters()))
    elif isinstance(input, torch.Tensor):
        sample = input
    else:
        raise ValueError("Only tensor.Torch or tuples are supported for the 'input' argument.")

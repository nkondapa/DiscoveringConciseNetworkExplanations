from interpretability_methods_src.zennit_crp.crp.concepts import *
...
class ChannelConcept(Concept):
    ...
    @staticmethod
    def attribute_unreduced(relevance, mask=None, layer_name: str = None):
        if isinstance(mask, torch.Tensor):
            relevance = relevance * mask

        unnormed_rel_l = torch.sum(relevance.view(*relevance.shape[:2], -1), dim=-1)
        abs_norm = (torch.abs(unnormed_rel_l).sum(-1).view(-1, 1) + 1e-10)

        return relevance, relevance / abs_norm, abs_norm
